    ...
    def test_model_outputs_equivalence(self):
        ...
        def check_equivalence(model, tuple_inputs, dict_inputs, additional_kwargs={}):
            with torch.no_grad():
                ...
                for tuple_value, dict_value in zip(tuple_output, dict_output):
                    if isinstance(tuple_value, (List, Tuple)):
                        for tuple_iterable_value, dict_iterable_value in zip(tuple_value, dict_value):
                            self.assertTrue(torch.allclose(tuple_iterable_value, dict_iterable_value))
                    else:
                        self.assertTrue(torch.allclose(tuple_value, dict_value))
        ...
        for model_class in self.all_model_classes:
            ...
            tuple_inputs = self._prepare_for_class(copy.deepcopy(inputs_dict), model_class)
            dict_inputs = self._prepare_for_class(copy.deepcopy(inputs_dict), model_class)
            check_equivalence(model, tuple_inputs, dict_inputs)

            tuple_inputs = self._prepare_for_class(copy.deepcopy(inputs_dict), model_class, return_labels=True)
            dict_inputs = self._prepare_for_class(copy.deepcopy(inputs_dict), model_class, return_labels=True)
            check_equivalence(model, tuple_inputs, dict_inputs)
